...
public class MemberController {
	...
	public void updateMember(String userId, String userPwd, int menu, String change) {
		...
		int result = new MemberDao().updateMember(userId, userPwd, menuType, change);
		if (result > 0) {
	...
	public void searchByUserId(String userId) {
		Member m = new MemberDao().searchByUserId(userId);
		if (m == null) {
	...
	public void searchByUserName(String keyword) {
		ArrayList<Member> list =new MemberDao().searchByUserName(keyword);
		if(list.isEmpty()) {
